...
> [!NOTE]
> If you are looking for the original template based on [this blog post](https://srid.ca/rust-nix)'s use of `crate2nix`, browse from [this tag](https://github.com/srid/hot_dog/tree/crate2nix). The evolution of this template can be gleaned from [releases](https://github.com/srid/hot_dog/releases).

## Usage

You can use [omnix](https://omnix.page/om/init.html)[^omnix] to initialize this template:
```
nix --accept-flake-config run github:juspay/omnix -- init github:gignsky/hot_dog -o ~/my-rust-project
```
...
![CI Nix](https://github.com/gignsky/hot_dog/actions/workflows/ci-nix.yml/badge.svg)](https://github.com/gignsky/hot_dog/actions/workflows/ci-nix.yml)
[![Docs](https://github.com/gignsky/hot_dog/actions/workflows/docs.yml/badge.svg)](https://github.com/gignsky/hot_dog/actions/workflows/docs.yml)
[![Release](https://github.com/gignsky/hot_dog/actions/workflows/release.yml/badge.svg)](https://github.com/gignsky/hot_dog/actions/workflows/release.yml)
[![Update README with --help Output](https://github.com/gignsky/hot_dog/actions/workflows/help-to-readme.yml/badge.svg)](https://github.com/gignsky/hot_dog/actions/workflows/help-to-readme.yml)
...
`$ nix run github:gignsky/hot_dog -- {optional-arguments (i.e. --help)}`

## Installation
### With Nix Shell (temporary installation in current shell)
`$ nix shell github:gignsky/hot_dog`

### With NixOS (using flakes)
In your `flake.nix` file: 
1. add `inputs.hot_dog.url = "github:gignsky/hot_dog";` to your inputs
...
3. add either:
   1. `inputs.hot_dog.packages.${system}.hot_dog`
   2. `inputs.hot_dog.packages.${system}.default`

### Download the binary
1. Navigate to https://github.com/gignsky/hot_dog/releases
...
1. git clone this repository:
   1. `$ git clone https://github.com/gignsky/hot_dog.git`
...
4. build binary with justfile (uses nix build command) or build with cargo
   1. `$ just build`
      1. nix build command
      2. produces a binary in `./result/bin/hot_dog`
   2. `$ cargo build`
      1. cargo build command (debug)
      2. produces a debug binary in `./target/debug/hot_dog`
   3. `$ cargo build --release`
      1. cargo build command (release)
      2. produces a debug binary in `./target/release/hot_dog`
